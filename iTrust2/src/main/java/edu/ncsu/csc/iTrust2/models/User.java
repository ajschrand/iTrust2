...
public class User extends DomainObject {
    ...
    public void setRoles ( final Set<Role> roles ) {
        ...
        if ( ( roles.contains( Role.ROLE_PATIENT ) || roles.contains( Role.ROLE_ADMIN )
                || roles.contains( Role.ROLE_BSM ) || roles.contains( Role.ROLE_PATIENTADVOCATE ) )
                && 1 != roles.size() ) {
            throw new IllegalArgumentException(
                    "Tried to create a Patient, Admin, Patient Advocate, or BSM user with a secondary role.  Patient, admin, Patient Advocate, and BSM can only have a single role!" );
        }
        if ( roles.size() == 0 ) {
            throw new IllegalArgumentException(
    ...
    public void addRole ( final Role role ) {
        ...
        if ( role.equals( Role.ROLE_ADMIN ) || role.equals( Role.ROLE_PATIENT )
                || role.equals( Role.ROLE_PATIENTADVOCATE ) ) {
            throw new IllegalArgumentException( "Admin, Patient, and Patient Advocate roles cannot be added" );
        }
        if ( this.roles.contains( Role.ROLE_ADMIN ) || this.roles.contains( Role.ROLE_PATIENT )
                || this.roles.contains( Role.ROLE_PATIENTADVOCATE ) ) {
            throw new IllegalArgumentException(
                    "Admins, Patients, and Patient Advocates cannot have additional roles added" );
        }
